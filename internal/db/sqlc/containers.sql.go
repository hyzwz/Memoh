...
SELECT id, bot_id, container_id, container_name, image, status, namespace, auto_start, container_path, created_at, updated_at, last_started_at, last_stopped_at, last_accessed_at FROM containers WHERE bot_id = $1 ORDER BY updated_at DESC LIMIT 1
`
...
SELECT id, bot_id, container_id, container_name, image, status, namespace, auto_start, container_path, created_at, updated_at, last_started_at, last_stopped_at, last_accessed_at FROM containers WHERE auto_start = true ORDER BY updated_at DESC
`
...
const listIdleRunningContainers = `-- name: ListIdleRunningContainers :many
SELECT id, bot_id, container_id, container_name, image, status, namespace, auto_start, container_path, created_at, updated_at, last_started_at, last_stopped_at, last_accessed_at FROM containers
WHERE status = 'running'
  AND (
    (last_accessed_at IS NOT NULL AND last_accessed_at < $1)
    OR (last_accessed_at IS NULL AND last_started_at IS NOT NULL AND last_started_at < $1)
    OR (last_accessed_at IS NULL AND last_started_at IS NULL AND created_at < $1)
  )
ORDER BY updated_at DESC
`

func (q *Queries) ListIdleRunningContainers(ctx context.Context, threshold pgtype.Timestamptz) ([]Container, error) {
	rows, err := q.db.Query(ctx, listIdleRunningContainers, threshold)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
const updateContainerAccessed = `-- name: UpdateContainerAccessed :exec
UPDATE containers
SET last_accessed_at = now(), updated_at = now()
WHERE bot_id = $1
`

func (q *Queries) UpdateContainerAccessed(ctx context.Context, botID pgtype.UUID) error {
	_, err := q.db.Exec(ctx, updateContainerAccessed, botID)
	return err
}
