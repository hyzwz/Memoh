...
const listBotMembershipsByUser = `-- name: ListBotMembershipsByUser :many
SELECT bot_id, user_id, role, created_at
FROM bot_members
WHERE user_id = $1
ORDER BY created_at DESC
...
func (q *Queries) ListBotMembershipsByUser(ctx context.Context, userID pgtype.UUID) ([]BotMember, error) {
	rows, err := q.db.Query(ctx, listBotMembershipsByUser, userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
