...
export const useChatStore = defineStore('chat', () => {
  ...
  function buildAssetBlocks(raw: Message): AttachmentBlock[] {
    ...
    const items: AttachmentItem[] = raw.assets.map((a) => ({
  ...
  function resolveIsSelf(raw: Message): boolean {
    ...
    const currentUserId = (appStore.userID ?? '').trim()
  ...
  function startWebSocket(targetBotId: string) {
    ...
    console.log('[desktop-chat:store] startWebSocket', { botId: bid })
  ...
  function handleLocalStreamEvent(event: StreamEvent) {
    ...
    switch (type) {
      ...
      case 'attachment_delta': {
        const items = event.attachments as AttachmentItem[] | undefined
  ...
  async function sendMessage(text: string, attachments?: ChatAttachment[]) {
    ...
    try {
      ...
      console.log('[desktop-chat:store] sendMessage', {
        botId: bid,
        chatId: cid,
        wsConnected: activeWs?.connected ?? false,
