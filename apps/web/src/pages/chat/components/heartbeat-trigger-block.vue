<template>
  <div class="rounded-xl border border-border bg-muted/40 p-4 text-sm">
    ...
    <div class="space-y-1 text-muted-foreground">
      <p v-if="meta.interval">
        <span class="font-medium text-foreground">Interval:</span> {{ meta.interval }}
      </p>
      <p v-if="meta.time">
        <span class="font-medium text-foreground">Time:</span> {{ meta.time }}
      </p>
      <p v-if="meta.last_heartbeat">
        <span class="font-medium text-foreground">Last:</span> {{ meta.last_heartbeat }}
      </p>
      <p
        v-if="body"
