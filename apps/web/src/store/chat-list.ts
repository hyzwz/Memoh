...
export const useChatStore = defineStore('chat', () => {
  ...
  const streaming = computed(() => streamingChatId.value !== null && streamingChatId.value === chatId.value)
  ...
  function hasMessageWithId(id: string) {
    const tid = id.trim()
    return tid ? knownServerMessageIds.has(tid) : false
  }
  ...
  function appendRealtimeMessage(raw: Message) {
    ...
    if (!item) return
    messages.push(item)
    if (mid) knownServerMessageIds.add(mid)
  ...
  async function loadMessages(botId: string, cid: string) {
    ...
    replaceMessages(items, rows.map((row) => String(row.id ?? '')))
