  ...
  <div class="p-6 space-y-6 mx-auto">
    ...
    <div class="flex items-center justify-between">
      <div>
        ...
        <p class="text-sm text-muted-foreground mt-1">管理组织架构，关联Bot与部门的访问权限</p>
      </div>
    ...
    <template v-if="!selectedBotId">
      <EmptyState icon="building" message="请先选择一个Bot管理部门" />
    </template>
    ...
    <template v-else-if="departments && departments.length > 0">
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        <Card v-for="dept in departments" :key="dept.id" class="relative overflow-hidden group">
          ...
          <CardContent class="p-5">
            <div class="flex items-start justify-between mb-3">
              <div class="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p v-if="dept.description" class="text-xs text-muted-foreground">{{ dept.description }}</p>
                </div>
              </div>
            </div>
            <div class="flex items-center justify-between text-xs text-muted-foreground">
              <div class="flex items-center gap-4">
                ...
                <span v-else class="text-muted-foreground/50">顶级部门</span>
              </div>
    ...
    <!-- Empty -->
    <template v-else>
      <EmptyState icon="building" message="暂无部门" sub="创建部门后可关联Bot访问权限和预算管理" />
    </template>
      ...
      <DialogContent class="sm:max-w-md">
        ...
        <div class="space-y-4 mt-4">
          <div class="space-y-1.5">
            <Label>部门名称</Label>
            <Input v-model="form.name" placeholder="例: 放射科" />
          </div>
          <div class="space-y-1.5">
            <Label>描述 <span class="text-muted-foreground text-xs">(可选)</span></Label>
            <Input v-model="form.description" placeholder="部门职责描述" />
          </div>
          <div class="space-y-1.5">
            <Label>上级部门 <span class="text-muted-foreground text-xs">(可选)</span></Label>
            <Select v-if="departments && departments.length > 0" v-model="form.parent_id">
              <SelectTrigger>
            ...
            <Input v-else v-model="form.parent_id" placeholder="上级部门ID (可选)" />
          </div>
        </div>
